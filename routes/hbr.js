...
async function main(url_news, uri_mostPopula, uri_study, uri_video) {
  try {
    ...
    page_news.setUserAgent(helpers.userAgent);
    ...
    page_mostPopula.setUserAgent(helpers.userAgent);
    await page_mostPopula.goto(uri_mostPopula, {
    ...
    for (const item of items_mostPopula) {
      try {
        ...
        if (x != null) {
          for (const autha of x) {
            let value = await page_mostPopula.evaluate(
              (li) => li.innerText,
              autha
            );
            authors.push(value);
    ...
    page_study.setUserAgent(helpers.userAgent);
    ...
    for (const item of items_video) {
      try {
        ...
        const url =
          h1 != null || undefined ? await h1.$eval("a", (a) => a.href) : null;
        const headline =
          h1 != null || undefined
            ? await h1.$eval("a", (a) => a.innerText)
            : null;
        const thumbnail = await item.$eval("img", (img) => img.src);
        const vidLen =
          wrapper != null || undefined
            ? await item.$eval("li.text-gray", (li) => li.innerText)
            : null;
        const date =
          wrapper != null || undefined
            ? await item.$eval("li.pubdate", (li) => li.innerText)
            : null;
        //
        const pam = await item.$(".pam");
        const a =
          pam != null || undefined
            ? await pam.$eval("stream-item", (a) => a.processesset.authors)
            : null;
        const lede =
          pam != null || undefined
            ? await pam.$eval("stream-item", (b) => b.processesset.summary)
            : null;
        const category =
          pam != null || undefined
            ? await pam.$eval("stream-item", (c) => c.processesset.topic)
            : null;
